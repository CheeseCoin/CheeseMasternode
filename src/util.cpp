...
int nAnonymizeCheeseAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Cheese
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Cheese
    // Mac: ~/Library/Application Support/Cheese
    // Unix: ~/.Cheese
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cheese";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Cheese.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cheesed.pid"));
