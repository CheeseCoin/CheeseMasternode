...
namespace {
    class CCheeseAddressVisitor : public boost::static_visitor<bool> {
    private:
        CCheeseAddress *addr;
    public:
        CCheeseAddressVisitor(CCheeseAddress *addrIn) : addr(addrIn) { }
...
bool CCheeseAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CCheeseAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CCheeseAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CCheeseAddressVisitor(this), dest);
}

bool CCheeseAddress::IsValid() const {
...
CTxDestination CCheeseAddress::Get() const {
    if (!IsValid())
...
bool CCheeseAddress::GetKeyID(CKeyID &keyID) const {
...
bool CCheeseAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CCheeseSecret::SetKey(const CKey& vchSecret) {
...
CKey CCheeseSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CCheeseSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CCheeseSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CCheeseSecret::SetString(const std::string& strSecret) {
