...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCheeseAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCheeseAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCheeseAmount = strAnonymizeCheeseAmount.remove(strAnonymizeCheeseAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCheeseAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCheeseAmount*COIN) nMaxToAnonymize = nAnonymizeCheeseAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCheeseAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCheeseAmount));
        strAnonymizeCheeseAmount = strAnonymizeCheeseAmount.remove(strAnonymizeCheeseAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCheeseAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
