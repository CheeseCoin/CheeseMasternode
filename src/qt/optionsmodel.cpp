...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCheeseAmount"))
        settings.setValue("nAnonymizeCheeseAmount", 1000);
    nAnonymizeCheeseAmount = settings.value("nAnonymizeCheeseAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeCheeseAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeCheeseAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeCheeseAmount:
            return QVariant(nAnonymizeCheeseAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeCheeseAmount:
            nAnonymizeCheeseAmount = value.toInt();
            settings.setValue("nAnonymizeCheeseAmount", nAnonymizeCheeseAmount);
            emit AnonymizeCheeseAmountChanged(nAnonymizeCheeseAmount);
            break;
        default:
            break;
        }
